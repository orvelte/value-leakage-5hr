...
def main():
    ...
    lines += [
        ...
        "",
        "**What the covert share is, and is not.** The narration is mostly *stance-faithful*: "
        "rollouts the judge marks INFLUENCED land favoured 0.879 of the time [0.718, 0.966] "
        "against 0.587 [0.432, 0.730] for deniers — bias +0.738 vs +0.202, Fisher OR 5.10, "
        "p=0.006. The model that says it is being swayed is, by and large, the model that is "
        "being swayed. The 12.6% figure is the part of the *deniers'* residual bias that the "
        "pigeonhole argument forces to be both real and unacknowledged. It is a lower bound on "
        "covert leakage, not a count of rollouts that lie. Worth contrasting explicitly with the "
        "paper's Claude models at 67–92% covert: on this task Qwen3.5-27B is mostly overt, and "
        "the interesting slice is small.",
        "",
    ...
    if pref:
        ...
        lines += ["| (none, reference) | **+0.420**  [+0.220, +0.622] |", "",
                  "Verbalizing the goal restores the full unprefilled effect. The denial, "
                  "against these two arms, does no more than a neutral opener — but both are at "
                  "floor, so the denial had no bias to remove. Unfiltered, neutral and denial "
                  "are identical at +0.017 each.",
                  "",
                  "**The denial tested against an *active* goal (A4).** Putting the denial after "
                  "the admission drops the bias to **+0.124** [−0.059, +0.325], while a "
                  "length-matched control that appends a neutral second sentence instead holds "
                  "at **+0.428** [+0.266, +0.603]. Difference **+0.303** [+0.057, +0.550], "
                  "p=0.016. So the denial does causal work once there is a goal to retract, and "
                  "the earlier “epiphenomenal” reading is downgraded to *does nothing beyond a "
                  "neutral opener when there is nothing to remove*.",
                  "",
                  "**It is the denial specifically, not any reversal (A10).** Replacing the "
                  "denial with a non-honesty reversal (“I'm not sure that's the right way to "
                  "approach this — let me just estimate”) gives **+0.366**, indistinguishable "
                  "from the no-reversal control (−0.062, p=0.61) and not from the denial arm "
                  "(+0.242, p=0.062). Re-admitting after the denial restores the effect "
                  "(+0.378). A pure last-stance-wins account does not fit: the non-honesty "
                  "reversal is also the last stance and it changes nothing.",
                  "",
                  "**Length is not the mechanism (A3/A9).** A contentless “Okay.” prefill keeps "
                  "the free-CoT length (8,226 median tokens) *and* the bias (+0.404). Two other "
                  "method sentences collapse the trace to ~1,200 tokens like the neutral arm yet "
                  "keep most of the bias — “Let me think about this carefully” +0.340, “Let me "
                  "break this into the component quantities and multiply them” +0.251, neither "
                  "distinguishable from the contentless arm. At a fixed ~1,200 tokens the bias "
                  "ranges from −0.045 to +0.340, so CoT length explains none of it, and the "
                  "neutral arm's null does not generalise to method sentences as a class.",
                  ""]
